...
def lottery_tra_football(term):
    # 抓包获得的API更改_term的参赛就可以获得历史数据
    # url='http://www.lottery.gov.cn/api/match_data.jspx?_term=17073&_ltype=9'
    url = 'http://www.lottery.gov.cn/api/match_data.jspx?_term={}&_ltype=9'.format(term)
    ...
    res=json[0]['match_vs']
    for i in res:
        print('%s : %s %s result : %s' % (i['awayTeamView'],i['homeTeamView'],i['results'],i['rb']))